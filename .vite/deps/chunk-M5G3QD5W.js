...
function assertIsArrayOfFunctions(
    ...
    if (!array.every((item) => typeof item === "function")) {
        const itemTypes = array
            .map((item) => (typeof item === "function" ? `function ${item.name || "unnamed"}()` : typeof item))
            .join(", ");
...
function configureStore(options) {
    ...
    if (finalMiddleware.some((item) => typeof item !== "function")) {
        throw new Error(
            false ? formatProdErrorMessage(4) : "each middleware provided to configureStore must be a function"
        );
    }
    ...
    if (storeEnhancers.some((item) => typeof item !== "function")) {
        throw new Error(
            false ? formatProdErrorMessage(7) : "each enhancer provided to configureStore must be a function"
        );
    }
...
function executeReducerBuilderCallback(builderCallback) {
    ...
    const builder = {
        addCase(typeOrActionCreator, reducer) {
            ...
            if (!type) {
                throw new Error(
                    false ? formatProdErrorMessage(28) : "`builder.addCase` cannot be called with an empty action type"
                );
            }
...
function buildCreateSlice({ creators } = {}) {
    ...
    return function createSlice2(options) {
        ...
        const reducers =
            (typeof options.reducers === "function" ? options.reducers(buildReducerCreators()) : options.reducers) ||
            {};
    ...
    const middleware = (api) => (next) => (action) => {
        ...
        const getOriginalState = () => {
            if (originalState === INTERNAL_NIL_TOKEN) {
                throw new Error(
                    false ? formatProdErrorMessage(23) : `${alm}: getOriginalState can only be called synchronously`
                );
            }
    ...
    const selector = Object.assign(
        function makeSelector(selectorFn, selectState) {
            return function selector2(state, ...args) {
                return selectorFn(
                    createStateProxy(selectState ? selectState(state, ...args) : state, reducerMap, initialStateCache),
                    ...args
                );
            };
        },
...
//# sourceMappingURL=chunk-M5G3QD5W.js.map
