...
function copyWithStructuralSharing(oldObj, newObj) {
    if (
        oldObj === newObj ||
        !((isPlainObject2(oldObj) && isPlainObject2(newObj)) || (Array.isArray(oldObj) && Array.isArray(newObj)))
    ) {
        return newObj;
    }
...
var shouldSkip = (skipSchemaValidation, schemaName) =>
    Array.isArray(skipSchemaValidation) ? skipSchemaValidation.includes(schemaName) : !!skipSchemaValidation;
        ...
        try {
            ...
            async function executeRequest(finalQueryArg) {
                ...
                if (forceQueryFn) {
                    ...
                    result = await endpointDefinition.queryFn(finalQueryArg, baseQueryApi, extraOptions, (arg2) =>
                        baseQuery(arg2, baseQueryApi, extraOptions)
                    );
                }
...
In the case of an unhandled error, no tags will be "provided" or "invalidated".`,
    ...
    function isForcedQuery(arg, state) {
        ...
        if (refetchVal) {
            return (
                refetchVal === true || (Number(/* @__PURE__ */ new Date()) - Number(fulfilledVal)) / 1e3 >= refetchVal
            );
        }
        return false;
    }
    ...
    const prefetch = (endpointName, arg, options) => (dispatch, getState) => {
        ...
        if (force) {
            ...
            const shouldRetrigger =
                (Number(/* @__PURE__ */ new Date()) - Number(new Date(lastFulfilledTs))) / 1e3 >= maxAge;
            if (shouldRetrigger) {
                dispatch(queryAction());
            }
        } else {
...
function buildSlice({
    ...
    const mutationSlice = createSlice({
        ...
        extraReducers(builder) {
            builder
                .addCase(mutationThunk.pending, (draft, { meta, meta: { requestId, arg, startedTimeStamp } }) => {
                    if (!arg.track) return;
    ...
    function buildMutationSelector() {
        return (id) => {
            ...
            const selectMutationSubstate = (state) =>
                selectApiState(state)?.mutations?.[mutationId] ?? defaultMutationSubState;
...
var buildBatchedActionsHandler = ({ api, queryThunk, internalState, mwApi }) => {
    ...
    function serializeSubscriptions(currentSubscriptions) {
        return JSON.parse(
            JSON.stringify(Object.fromEntries([...currentSubscriptions].map(([k, v]) => [k, Object.fromEntries(v)])))
        );
    }
    return (action, mwApi2) => {
        ...
        if (didMutate) {
            ...
            const isSubscriptionSliceAction =
                typeof action.type == "string" && !!action.type.startsWith(subscriptionsPrefix);
        ...
        if (!anySubscriptionsRemainingForKey(queryCacheKey)) {
            ...
            currentRemovalTimeouts[queryCacheKey] = setTimeout(() => {
                if (!anySubscriptionsRemainingForKey(queryCacheKey)) {
                    ...
                    if (entry?.endpointName) {
                        const runningQuery = api2.dispatch(
    ...
    function handleNewKey(endpointName, originalArgs, queryCacheKey, mwApi, requestId) {
        ...
        const lifecycleApi = {
            ...
            updateCachedData: isAnyQueryDefinition(endpointDefinition)
                ? (updateRecipe) => mwApi.dispatch(api.util.updateQueryData(endpointName, originalArgs, updateRecipe))
                : void 0,
            cacheDataLoaded,
            cacheEntryRemoved,
        };
        ...
        if (isPendingThunk(action)) {
            ...
            if (onQueryStarted) {
                ...
                const lifecycleApi = {
                    ...
                    updateCachedData: isAnyQueryDefinition(endpointDefinition)
                        ? (updateRecipe) =>
                              mwApi.dispatch(api.util.updateQueryData(endpointName, originalArgs, updateRecipe))
                        : void 0,
                    queryFulfilled,
                };
        ...
        context.batch(() => {
            for (const queryCacheKey of subscriptions.keys()) {
                ...
                const shouldRefetch =
                    ...
                    (values.every((sub) => sub[type] === void 0) && state.config[type]);
                if (shouldRefetch) {
                    if (subscriptionSubState.size === 0) {
                        api2.dispatch(
                            removeQueryResult({
    ...
    const middleware = (mwApi) => {
        ...
        return (next) => {
            return (action) => {
                ...
                const [actionShouldContinue, internalProbeResult] = batchedActionsHandler(
                    action,
                    mwApiWithNext,
                    stateBefore
                );
