...
function isDataStrategyResults(result) {
    return (
        ...
        Object.entries(result).every(([key, value]) => typeof key === "string" && isDataStrategyResult(value))
    );
}
...
function useRevalidator() {
    ...
    return React2.useMemo(() => ({ revalidate, state: state.revalidation }), [revalidate, state.revalidation]);
}
function useMatches() {
...
function UNSTABLE_TransitionEnabledRouterProvider({ router, flushSync: reactDomFlushSyncImpl, unstable_onError }) {
    ...
    React3.useLayoutEffect(() => {
        return router.subscribe((newState, { deletedFetchers, flushSync: flushSync2, viewTransitionOpts }) => {
...
async function getKeyedPrefetchLinks(matches, manifest, routeModules) {
    ...
    return dedupeLinkDescriptors(
        links
            ...
            .map((link) =>
                link.rel === "stylesheet" ? { ...link, rel: "prefetch", as: "style" } : { ...link, rel: "prefetch" }
            )
    );
}
    ...
    return (routesByParentId[parentId] || []).map((route) => {
        ...
        if (routeModule) {
            ...
            let isHydrationRequest =
                needsRevalidation == null && (routeModule.clientLoader?.hydrate === true || !route.hasLoader);
...
async function fetchAndApplyManifestPatches(
    ...
    parentIds.forEach((parentId) =>
        patchRoutes(parentId || null, createClientRoutes(patches, routeModules, null, ssr, isSpaMode, parentId))
    );
}
...
function PrefetchPageLinksImpl({ page, matches: nextMatches, ...linkProps }) {
    ...
    }, [basename, loaderData, location2, manifest, newMatchesForData, nextMatches, page, routeModules]);
...
${matches
    .map((match, routeIndex) => {
        ...
        let { clientActionModule, clientLoaderModule, clientMiddlewareModule, hydrateFallbackModule, module } =
            manifestEntry;
        let chunks = [
            ...(clientActionModule
                ? [
...
function RemixRootDefaultErrorBoundary({ error, isOutsideRemixApp }) {
    ...
    if (isRouteErrorResponse(error)) {
        return React9.createElement(
            ...
            React9.createElement("h1", { style: { fontSize: "24px" } }, error.status, " ", error.statusText),
...
var Link = React10.forwardRef(function LinkWithRef(
    ...
    return shouldPrefetch && !isAbsolute
        ? React10.createElement(React10.Fragment, null, link, React10.createElement(PrefetchPageLinks, { page: href2 }))
        : link;
});
    ...
    if (typeof document !== "undefined") {
        ...
        React10.useLayoutEffect(() => {
            let disableScrollRestoration = router?.enableScrollRestoration(
                ...
                () => window.scrollY,
                getKey
                    ? (location22, matches2) => getScrollRestorationKey(location22, matches2, basename, getKey)
                    : void 0
            );
        ...
        React10.useLayoutEffect(() => {
            ...
            try {
                ...
                warning(
                    false,
                    `"${location2.hash.slice(1)}" is not a decodable element ID. The view will not scroll to it.`
                );
            }
...
function ServerRouter({ context, url, nonce }) {
    ...
    for (let match of context.staticHandlerContext.matches) {
        ...
        if (
            route &&
            manifestRoute &&
            shouldHydrateRouteLoader(routeId, route.clientLoader, manifestRoute.hasLoader, context.isSpaMode) &&
                        ...
                        return () => {
                            ...
                            window.__reactRouterDataRouter._internalSetStateDoNotUseOrYouWillBreakYourApp({
                                ...
                                errors: rerender.errors
                                    ? Object.assign({}, globalVar.__reactRouterDataRouter.state.errors, rerender.errors)
                                    : null,
                            });
                        };
                    }
...
function createRouterFromPayload({ fetchImplementation, createFromReadableStream, getContext, payload }) {
    ...
    globalVar.__reactRouterDataRouter = createRouter({
        ...
        async patchRoutesOnNavigation({ path, signal }) {
            ...
            await fetchAndApplyManifestPatches2([path], createFromReadableStream, fetchImplementation, signal);
        },
...
react-router/dist/development/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-UJSIC2R3.js.map
